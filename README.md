...
node cli/index.mjs publish --workspace ./runs/acme          # push a shareable playground.wordpress.net preview link
node cli/index.mjs publish --workspace ./runs/acme --clobber   # re-push the preview after changes (same asset name)
```
...
- **`--fast`** treats the brochure's generated design as a *suggestion* — the user never sees the mockup, so pages gate on sanity (tree serializes, renders, covers every mockup section) instead of pixel parity, with mismatch metrics reported as information. Judgment calls route to a fast model (`sonnet`), pages run their design→author→check chains fully in parallel, the site design's tokens land in `theme.json`, the theme assembles deterministically (zero theme-plan calls), and the Playground render gates on **render QA**: a vision judge lists anything visibly broken in the real WordPress screenshots (unreadable text, overlapping or orphaned elements, overflow, wrapped navigation, blank boxes — defects, not taste) and a bounded loop fixes until the list is empty or stops shrinking. Imports and non-fast brochure keep the strict pixel gates — see `docs/cli.md` (“Fast mode”).
...
- **`wbdc publish`** packages the same site as a self-booting Playground bundle, pushes it to a `playground-artifacts` branch on the repo's GitHub origin, and prints a shareable `playground.wordpress.net` link — anyone opening it gets the full site running in their browser, no server anywhere. The repo must be public (Playground fetches the ZIP from `raw.githubusercontent.com`); the bundle ships only the theme and plugins, never logs. Re-pushing the same asset name needs `--clobber`; `--repo OWNER/REPO` overrides the origin-derived repo, `--name`/`--out` control the asset filename and local bundle path, and `--dry-run` builds the bundle without pushing.
