...
- **`--fast`** is the speed preset: judgment calls route to a fast model (`sonnet`), brochure pages run their design→build→repair chains fully in parallel (plan+author merged, no foundation barrier), concurrency rises to 6, repairs cap at one round (best build always kept, rolled back on regression), and the brochure theme is assembled deterministically from the chrome splice — zero theme-plan calls, with every creative artifact shipped verbatim. The fidelity gates and thresholds are unchanged — see `docs/cli.md` (“Fast mode”) for per-role `--model-*`/`--effort-*` overrides.
